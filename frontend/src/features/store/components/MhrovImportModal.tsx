...
import { importMhrovsFromCsv } from "../api/store.api";
import { Button } from "@/components/ui/button";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription } from "@/components/ui/dialog";
import { UploadCloud, FileText, CheckCircle, AlertCircle } from "lucide-react";
import { toast } from "sonner";

interface MhrovImportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}
...
const IMPORT_STAGES = [
  ...
  { progress: 80, label: "Linking to Store Receipts..." },
  { progress: 90, label: "Finalising import..." },
];

export function MhrovImportModal({ isOpen, onClose, onSuccess }: MhrovImportModalProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await importMhrovsFromCsv(file);
  ...
  const downloadSampleCsv = () => {
    const headers = "MHROV No,MHROV Date,Status,Package,Circle,DI No,Vendor Name,Invoice No,Item Name,LOA Serial No,Temp Code,MHROV Done Qty\n";
    const sampleRow1 = "MHR-0001,2026-08-20,done,PKG-1,North,DI-001,Fastigo Tech,PINV-10001,Optical Fiber,LOA-1234,FBR-001,5\n";
    const sampleRow2 = "MHR-0001,2026-08-20,done,PKG-1,North,DI-001,Fastigo Tech,PINV-10001,Router,LOA-1234,RTR-900,2\n";
    const blob = new Blob([headers + sampleRow1 + sampleRow2], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.setAttribute("href", url);
    link.setAttribute("download", "mhrov_import_sample.csv");
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && !isUploading && onClose()}>
      <DialogContent className="sm:max-w-[500px] bg-white">
        <DialogHeader>
          <DialogTitle className="text-xl">Import MHROVs</DialogTitle>
          <DialogDescription>
            Upload a CSV file. Multiple rows with the same MHROV No will be grouped into one MHROV record.
          </DialogDescription>
        </DialogHeader>
