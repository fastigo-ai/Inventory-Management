        ...
        <div className="overflow-x-auto print:overflow-visible">
          <table className="w-full text-sm text-left whitespace-nowrap print:whitespace-normal print:text-xs">
            ...
            <tbody className="divide-y divide-slate-100">
              {demandNote.items && demandNote.items.filter((item: any) => Number(item.demandQty) > 0).length > 0 ? (
                Object.entries(
                  demandNote.items.filter((item: any) => Number(item.demandQty) > 0).reduce((acc: any, item: any, originalIdx: number) => {
