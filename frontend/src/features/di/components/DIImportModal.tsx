...
import { importDIsFromCsv } from "@/features/di/api/di.api";
...
export function DIImportModal({ isOpen, onClose, onSuccess }: DIImportModalProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (res.data.successCount > 0) {
        toast.success(`Imported successfully! ${res.data.successCount} DIs added.`);
        onSuccess();
  ...
  const downloadSampleCsv = () => {
    ...
    const sampleRow1 = "DI-20001,2026-07-21,PO-10001,Mumbai,Hardware Pack 1,Draft,First delivery batch,Optical Fiber,FBR-001,SN-1234,Hardware Pack 1,Mumbai,2\n";
    const sampleRow2 = "DI-20001,2026-07-21,PO-10001,Mumbai,Hardware Pack 1,Draft,First delivery batch,Router,RTR-002,SN-9988,Hardware Pack 1,Mumbai,5\n";
    ...
    link.setAttribute("download", "di_sample.csv");
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[500px] bg-white">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload a CSV file containing your Dispatch Instructions. Note that multiple rows with the same DINumber will be grouped into one DI.
          </DialogDescription>
        </DialogHeader>
              ...
              {result.successCount > 0 && (
                <div className="p-4 bg-green-50 border border-green-200 rounded-lg flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-green-700">{result.successCount} DIs added</p>
                  </div>
                </div>
              )}
