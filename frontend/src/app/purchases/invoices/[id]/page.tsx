...
export default function PurchaseInvoiceDetailPage() {
    ...
    const fetchInvoice = async () => {
      try {
        ...
        if (data) {
          ...
          data.invoiceNumber = data.PurchaseInvoiceNumber;
  ...
  const handleApproveReceipt = async () => {
    if (window.confirm('Are you sure you want to approve this Invoice and receive items into inventory?')) {
      try {
        const { updatePurchaseInvoice } = await import('@/features/purchases/api/purchases.api');
        await updatePurchaseInvoice(id, { status: 'Received' });
          ...
          {receives
            ...
            >
              <div className="flex justify-between items-start mb-1">
                <span className={`font-semibold ${pr._id === id ? 'text-blue-700' : 'text-slate-800'}`}>{pr.PurchaseInvoiceNumber}</span>
      ...
      <div className="flex flex-col 2xl:flex-row 2xl:items-center justify-between px-6 py-4 bg-white border-b border-slate-200 shrink-0 gap-y-4 print:hidden">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button 
            ...
            onClick={() => router.push(`/purchases/invoices/${id}/edit`)}
          >
