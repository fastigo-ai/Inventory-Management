...
// I will insert an auto-populate useEffect right after the combinedItems logic.
const insertionPoint = `  const uniqueActivities = useMemo(() => {`;

const autoPopulateLogic = `  // Auto-populate line items when a JMC is selected
  useEffect(() => {
...
if (!code.includes('Auto-populate line items')) {
  code = code.replace(insertionPoint, autoPopulateLogic);
  fs.writeFileSync(path, code);
  console.log("Applied auto-populate logic");
} else {
