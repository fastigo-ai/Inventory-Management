...
mongoose.connect(process.env.MONGO_URI).then(async () => {
    ...
    const locRow = rows[6]; // 'Location :'
    
    const excelLocations = [];
    if (locRow) {
        for(let i=3; i<locRow.length; i++) {
            if(locRow[i]) {
                excelLocations.push(locRow[i].toString().trim().toUpperCase());
            } else {
                excelLocations.push('UNKNOWN_' + i);
            }
        }
    }
    ...
    console.log('\n--- In DB but NOT in Excel (System has these, Excel does not) ---');
    console.dir(inDbNotExcel, {maxArrayLength: null});
    
    console.log('\n--- In Excel but NOT in DB (Excel has these, System does not) ---');
    console.dir(inExcelNotDb, {maxArrayLength: null});
