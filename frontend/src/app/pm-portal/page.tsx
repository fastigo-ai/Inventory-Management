...
export default function PMPortalDashboard() {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        const response = await api.get('/dashboard/pm-portal-summary');
        if (response.data.success) {
          setData(response.data.data);
        }
      } catch (error) {
        toast.error('Failed to load PM Dashboard data');
      } finally {
        setIsLoading(false);
      }
    };
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-slate-50 p-6 md:p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm">
          <h1 className="text-3xl font-bold text-slate-800 tracking-tight">Project Manager Dashboard</h1>
          <p className="text-slate-500 mt-2">Overview of your assigned Package and Circle.</p>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Link href="/pm-portal/demand-notes?tab=pending" className="block group">
