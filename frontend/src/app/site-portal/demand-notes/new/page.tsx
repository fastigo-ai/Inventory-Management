...
export default function CreateDemandNotePage() {
  ...
  const [formData, setFormData] = useState({
    ...
    location: '',
    remarks: '',
    status: 'Pending Approval' // Submitting directly puts it in Pending Approval
  });
  ...
  const addItemRow = () => {
    setItems([
      ...items,
      {
        ...
        jmcQty: 0, wipQty: 0, demandQty: 0, balBomQty: 0, isLoadingContext: false
      }
    ]);
  };
  ...
  const handleItemSelect = async (index: number, itemId: string) => {
    ...
    try {
      ...
      setItems([...newItems]); // Force re-render
    }
  };
      ...
      <div className="flex justify-between items-center bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">Package & Circle will be automatically captured from your profile.</p>
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {items.map((item, idx) => (
                <tr key={idx} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">
                    {item.isLoadingContext ? <Loader2 className="w-4 h-4 animate-spin text-indigo-500" /> : <span className="text-slate-600 truncate max-w-[150px] inline-block" title={item.itemDescription}>{item.itemDescription || '-'}</span>}
                  </td>
                  ...
                  <td className="px-4 py-3 font-medium text-amber-600">{item.alreadyIssuedQty}</td>
                  <td className="px-4 py-3">
                    <Input
                      type="number"
                      className="w-24 h-9"
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      <Input type="number" className="w-16 h-9" placeholder="JMC" value={item.jmcQty || ''} onChange={(e) => handleItemChange(idx, 'jmcQty', e.target.value)} />
                      <Input type="number" className="w-16 h-9" placeholder="WIP" value={item.wipQty || ''} onChange={(e) => handleItemChange(idx, 'wipQty', e.target.value)} />
                    </div>
                  </td>
