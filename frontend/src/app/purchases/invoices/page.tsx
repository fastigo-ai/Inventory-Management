...
import { getPurchaseInvoices, exportPurchaseInvoicesToCsv, getUniqueVendors, getPIItemSummary } from "@/features/purchases/api/purchases.api";
...
export default function PurchaseInvoicesPage() {
  ...
  useEffect(() => {
    if (activeTab === 'analytics' && summaryData.length === 0) fetchSummaryData();
  }, [activeTab]);
  ...
  return (
    <div className="flex flex-col h-full bg-[#fcfcfc] overflow-hidden">
      ...
      {/* Tab Navigation */}
      <div className="px-6 border-b border-slate-200 bg-white flex space-x-6 shrink-0">
        <button
      ...
      {/* Main Content Area */}
      {activeTab === 'list' && <div className="flex-1 overflow-y-auto px-6 pb-6 flex flex-col space-y-5 bg-[#fcfcfc]">
              ...
              {isSummaryLoading ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={summaryData.slice(0, 15)} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    ...
                    <RechartsTooltip formatter={(val) => new Intl.NumberFormat('en-IN').format(Number(val))} />
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  {isSummaryLoading ? (
                    ...
                    <tr><td colSpan={5} className="py-20 text-center text-slate-500">No items found.</td></tr>
                  ) : (
                    summaryData
                      .filter(item =>
                        (item.tempCode?.toLowerCase() || '').includes(summarySearch.toLowerCase()) ||
