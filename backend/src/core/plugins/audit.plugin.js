...
import AuditLog, { AuditAction } from '../../modules/audit/auditLog.model';
...
export function auditPlugin(schema, options) {
    ...
    const createLog = async (docId, action, changes, moduleName) => {
        if (changes.length === 0 && action === AuditAction.UPDATE)
            return; // Don't log empty updates
